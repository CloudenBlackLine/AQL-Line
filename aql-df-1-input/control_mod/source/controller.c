...
#else
	if(motion_sensor_update(&ctrl.raw_data) != IMU_OK) return;
	// Raw input only. Real angle estimation is still needed.
#endif
...
{
#if !SIMULATOR_ON
	/* Hardware is input-only until real angle estimation is added. */
	return;
#endif
	// P calucation
